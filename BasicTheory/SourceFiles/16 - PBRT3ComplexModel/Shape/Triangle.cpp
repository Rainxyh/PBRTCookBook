...
namespace Feimos
{
							 ...
							 bool testAlphaTexture) const
	{
		...
		// 三 角 形 表 面 偏 微 分， 用 于 计 算 光 线 微 分
		Vector3f dpdu, dpdv;
		Point2f uv[3];
		GetUVs(uv);
		...
		*isect = SurfaceInteraction(pHit, pError, uvHit, -ray.d, dpdu, dpdv,
		...
		if (mesh->n || mesh->s)
		{
			...
			if (mesh->n)
			{
				// 计 算 三 角 形 的 着 色 几 何 切 线 向 量 dndu 和 副 切 线 向 量 dndv
				// Compute deltas for triangle partial derivatives of normal
				Vector2f duv02 = uv[0] - uv[2]; // 纹理坐标向量
				...
				else // 2X2矩 阵 中 通 过 行 列 式 和 伴 随 矩 阵 的 方 法 求 逆 会 比 较 快
				{
					float invDet = 1 / determinant;
