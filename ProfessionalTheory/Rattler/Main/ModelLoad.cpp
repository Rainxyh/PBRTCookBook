...
namespace Rattler
{
	...
	void ModelLoad::processMesh(aiMesh *mesh, const aiScene *scene)
	{
		...
		// 写入文件
		{
			...
			// (3) 纹理的名字，根据透明度来制作透明纹理，将纹理根据Mesh进行编号
			{
				...
				if (diffTexName.size() > 0)
				{
					...
					if (isGetDiffuse)
						file << "Diffuse " << diffOutName << std::endl;
					else
						file << "Diffuse "
							 << "NoFile_Rattler" << std::endl;

					if (isGetAlpha)
						file << "Alpha " << alphaOutName << std::endl;
					else
						file << "Alpha "
							 << "NoFile_Rattler" << std::endl;
					...
					if (specTexName.size() > 0)
					{
						...
						if (isGetSpecular)
							file << "Specular " << specOutName << std::endl;
						else
							file << "Specular "
								 << "NoFile_Rattler" << std::endl;
					}
		...
		if (ModelMinPos[0] > minPos[0])
			ModelMinPos[0] = minPos[0];
		if (ModelMinPos[1] > minPos[1])
			ModelMinPos[1] = minPos[1];
		if (ModelMinPos[2] > minPos[2])
			ModelMinPos[2] = minPos[2];

		if (ModelMaxPos[0] < maxPos[0])
			ModelMaxPos[0] = maxPos[0];
		if (ModelMaxPos[1] < maxPos[1])
			ModelMaxPos[1] = maxPos[1];
		if (ModelMaxPos[2] < maxPos[2])
			ModelMaxPos[2] = maxPos[2];
	...
	void ModelLoad::loadModel(std::string path, std::string outDir)
	{
...
#else
		int warn_unused_result = system(("mkdir " + outputDir).c_str());
#endif
		...
		{
			file << "MeshFile "
				 << "/MeshInfo_" + std::to_string(i) + ".ratr" << std::endl;
		}
