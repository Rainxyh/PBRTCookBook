...
void RenderThread::run()
{
	...
	{
		// 初始化程序
		Feimos::Point3f eye(0.f, 0.f, 3.7f), look(0.f, 0.f, 0.0f);
	...
	emit PrintString("Init Floor...");
	{
		...
		Feimos::Transform tri_Floor2World = Feimos::Translate(Feimos::Vector3f(-0.5 * length_Wall, -2.56, -0.5 * length_Wall));
		Feimos::Transform tri_World2Floor = Feimos::Inverse(tri_Floor2World);
		std::shared_ptr<Feimos::TriangleMesh> meshConBox = std::make_shared<Feimos::TriangleMesh>(tri_Floor2World, nTrianglesWall, vertexIndicesWall, nVerticesWall, P_Wall, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr);
	...
	emit PrintString("Init Model");
	{
		...
		// Feimos::Transform tri_Object2WorldModel = Feimos::Translate(Feimos::Vector3f(0.0f, -0.5f, 0.0f)) * Feimos::RotateY(180) * Feimos::Scale(3.003f, 3.003f, 3.003f);
		// Feimos::RattlerLoad rattler("../../Resources/Rattler/Wall-Rattler/", tri_Object2WorldModel, prims, noMedium);
	...
	std::vector<std::shared_ptr<Feimos::Light>> lights;
	{
		...
		// 面光源的变换矩阵
		Feimos::Transform tri_Object2World_AreaLight = Feimos::Translate(Feimos::Vector3f(0.0f, 2.45f, 2.0f)) * Feimos::RotateX(45) * Feimos::Scale(0.5, 0.5, 0.5);
		...
		for (int i = 0; i < nTrianglesAreaLight; ++i)
		{
			std::shared_ptr<Feimos::AreaLight> area =
				std::make_shared<Feimos::DiffuseAreaLight>(tri_Object2World_AreaLight, noMedium, Feimos::Spectrum(135.f), 5, trisAreaLight[i], false);
