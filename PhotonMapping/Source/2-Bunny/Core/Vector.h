...
inline vec3 HDRtoLDR(const vec3& col, float exposure) {
	exposure = 1.0 - exposure;
	float invExposure = 1.0 / exposure;
	float gamma = 2.2;
	//HDRÐÞÕý
	vec3 temp_c;
	temp_c[0] = 1.0 - expf(-col[0] * invExposure);
	//temp_cx = powf(temp_cx, 1.0 / gamma);
	temp_c[1] = 1.0 - expf(-col[1] * invExposure);
	//temp_cy = powf(temp_cy, 1.0 / gamma);
	temp_c[2] = 1.0 - expf(-col[2] * invExposure);
